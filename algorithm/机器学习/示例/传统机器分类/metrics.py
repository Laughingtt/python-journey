...
def map_ci(ci):
    return [float(f"{i_:.6f}") for i_ in ci]
...
def IDI(gt, pred_x, pred_y, with_p: bool = False):
    """
    ...
    if with_p:
        return idi, idi / ((event_x_y.std() ** 2 + non_event_x_y.std() ** 2) ** 0.5)
    else:
        return idi
...
def NRI(pred_x: Union[List, np.ndarray, pd.DataFrame], pred_y: Union[List, np.ndarray, pd.DataFrame],
    ...
    """
    y_true = column_or_1d(np.array(y_true))
    pred_x = column_or_1d(np.array(pred_x))
    len_labels = len(np.unique(y_true))
    pred_y = column_or_1d(np.array(pred_y))
    ...
    a = (np.sum(matrix_event * triu) - np.sum(matrix_event * tril)) / event_num
    b = (np.sum(matrix_non_event * tril) - np.sum(matrix_non_event * triu)) / non_event_num
    return a + b
