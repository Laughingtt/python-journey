...
#     )
#
#     reranker_args = {'model': '/mnt/workspace/llm/bce/bce-reranker-base_v1', 'top_n': 2, 'device': 'cuda:0'}
...
#         search_type="similarity", search_kwargs={"score_threshold": 0.3, "k": 6})
#
...
#
#     # 运行 chain
#     qa_chain = RetrievalQA.from_chain_type(llm=conversation_llm, retriever=compression_retriever,
...
with block as demo:
    with gr.Row(equal_height=True):
        gr.Image('yingying.webp', width=100, scale=0)
    ...
    with gr.Row():
        with gr.Column(scale=4):
            # 创建一个聊天机器人对象
            chatbot = gr.Chatbot(height=850, show_copy_button=True, avatar_images=("xiaobai.png", "yingying.webp"),
                                 label="唠五毛")
            first = """
### 唠五毛 - 为你提供情绪价值的智能机器人
    ...
    我最近学习成绩下降，感觉很难集中注意力，这让我很焦虑。我该怎么办呢？"""
            ...
            with gr.Row():
                ...
                msg = gr.Textbox(label="问题", lines=3, placeholder="点击发送", scale=20,
                                 show_label=False)
                db_wo_his_btn = gr.Button("发送", scale=1, icon="send.webp")
